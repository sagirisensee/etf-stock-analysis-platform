...
def remove_from_user_pool(user_id, pool_id):
    ...
    with get_db() as conn:
        conn.execute(
            "DELETE FROM stock_pools WHERE id = ? AND user_id = ?", (pool_id, user_id)
        )
        conn.commit()
...
def save_user_analysis_history(user_id, analysis_type, results):
    ...
    try:
        ...
        for i, item in enumerate(results):
            ...
            except TypeError as item_e:
                logger.error(f"问题在第{i}个项目: {item_e}")
                logger.error(f"问题项目内容: {item}")
                break
        raise e
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (user_id, analysis_type, json_str),
        )
        conn.commit()
    ...
    with get_db() as conn:
        ...
        try:
            ...
            try:
                ...
                try:
                    # 检查stock_pools表是否有正确的结构
                    conn.execute(
                        "SELECT user_id, name, type, code FROM stock_pools LIMIT 1"
                    )
                except sqlite3.OperationalError:
    ...
    with get_db() as conn:
        ...
        if not admin_user:
            ...
            existing_config = conn.execute(
                "SELECT 1 FROM user_configs WHERE user_id = ? LIMIT 1", (admin_id,)
            ).fetchone()
...
def get_stock_pools(pool_type=None):
    ...
    with get_db() as conn:
        if pool_type:
            results = conn.execute(
                "SELECT * FROM stock_pools WHERE type = ? ORDER BY name", (pool_type,)
            ).fetchall()
        else:
            results = conn.execute(
...
def add_to_pool(name, pool_type, code):
    ...
    with get_db() as conn:
        ...
        if existing_code:
            raise ValueError(
                f"该{pool_type}代码已存在：{existing_code['name']} ({code})"
            )
        ...
        if existing_name:
            raise ValueError(
                f"该{pool_type}名称已存在：{name} ({existing_name['code']})"
            )
...
def api_pools():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "etf_pools": etf_pools, "stock_pools": stock_pools}
        )
    except Exception as e:
        logger.error(f"获取池子数据失败: {e}", exc_info=True)
        return jsonify({"success": False, "error": f"获取池子数据失败: {str(e)}"}), 500
    ...
    try:
        ...
        if individual_code and individual_name:
            ...
            if analysis_type in ["etf", "etf_debug"]:
                core_pool = [
                    {"code": individual_code, "name": individual_name, "type": "etf"}
                ]
            elif analysis_type in ["stock", "stock_debug"]:
                core_pool = [
                    {"code": individual_code, "name": individual_name, "type": "stock"}
                ]
            else:
            ...
            if analysis_type == "etf":
                ...
                if not pools:
                    return jsonify(
                        {"success": False, "error": "ETF标的池为空，请先添加ETF标的"}
                    )
                core_pool = [
                    {"code": p["code"], "name": p["name"], "type": "etf"} for p in pools
                ]
                ...
                if not pools:
                    return jsonify(
                        {
                            "success": False,
                            "error": "个人股票标的池为空，请先添加股票标的",
                        }
                    )
                core_pool = [
                    {"code": p["code"], "name": p["name"], "type": "stock"}
                    for p in pools
                ]
            else:
                return jsonify({"success": False, "error": "不支持的分析类型"})
        ...
        try:
            if analysis_type in ["etf", "etf_debug", "etf_debug_with_details"]:
                ...
                if analysis_type.startswith("stock_debug"):
                    results = loop.run_until_complete(
                        get_detailed_analysis_report_for_debug(
                            get_all_stock_spot_realtime,
                            get_stock_daily_history,
...
def get_history_detail(record_id):
    ...
    try:
        ...
        with get_db() as conn:
            ...
            try:
                results_data = (
                    json.loads(result["results"]) if result["results"] else []
                )
            except:
                results_data = []
...
def clear_all_history():
    ...
    try:
        ...
        with get_db() as conn:
            ...
            return jsonify(
                {"success": True, "message": f"已清空 {deleted_count} 条历史记录"}
            )
        ...
        async def test_request():
            try:
                ...
                if "model" in error_msg and (
                    ...
                    raise Exception(
                        f"模型 '{model_name}' 不存在或无效，请检查模型名称是否正确"
                    )
                elif (
...
def export_pools():
    ...
    try:
        ...
        export_data = {
            ...
            "stock_pools": [
                {"name": p["name"], "code": p["code"]} for p in stock_pools
            ],
        }
...
def import_pools():
    ...
    try:
        ...
        if imported_count > 0 and skipped_count > 0:
            flash(
                f"成功导入 {imported_count} 个标的，跳过 {skipped_count} 个重复标的",
                "success",
            )
